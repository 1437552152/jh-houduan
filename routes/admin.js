...
router.post('/team/update', function (req, res) {
    ...
    let Id = req.body.Id;
    let sql = "UPDATE student set studentname=?,education=?,school=?,teachname=?,pic=?,introduceBriefly=?,languagelevel=?,address=?,score=?,language=?,content=? where id=?"
    var param = [studentname, education, school, teachname, pic, introduceBriefly, languagelevel, address, score, language, content, Id];
...
router.post('/team/add', function (req, res) {
    ...
    let content = req.body.content;
    let isShow = 0;
    let sql = "insert  into student(studentname,education,school,teachname,pic,introduceBriefly,languagelevel,address,score,language,content,isShow) values(?,?,?,?,?,?,?,?,?,?,?,?)"
    var param = [studentname, education, school, teachname, pic, introduceBriefly, languagelevel, address, score, language, content, isShow];
    db.query(sql, param, function (err, results) {
        if (err) {} else {
            res.json({
                msg: '操作成功',
...
router.post('/countryconfig', function (req, res) {
    ...
    let sql2 = "SELECT*FROM countryconfig where isShow=0 limit" + " " + (pageNo - 1) * pageSize + "," + (pageNo * pageSize);
    db.query(sql, function (err, results) {
        if (err) {} else {
            allCount = results[0]['COUNT(*)'];
            back(allCount)
...
router.post('/countryconfig/add', function (req, res) {
    ...
    let isShow = 0;
    let sql = "insert  into countryconfig(logo,pic1,pic2,pic3,country,typeid,isShow) values(?,?,?,?,?,?,?)"
...
router.post('/countryconfig/update', function (req, res) {
    ...
    let country = req.body.country;
    let Id = req.body.Id;
    let sql = "UPDATE countryconfig set logo=?,pic1=?,pic2=?,pic3=?,typeid=?,country=?  where Id=?"
...
router.post('/news', function (req, res) {
    ...
    let sql2 = "SELECT*FROM news where isShow=0 limit" + " " + (pageNo - 1) * pageSize + "," + (pageNo * pageSize);
    db.query(sql, function (err, results) {
        if (err) {} else {
            allCount = results[0]['COUNT(*)'];
            back(allCount)
...
router.post('/news/update', function (req, res) {
    ...
    let content = req.body.content;
    let Id = req.body.Id;
    let sql = "UPDATE news SET title=?,author=?,des=?,keyword=?,focusPic=?,newstype=?,language=?,content=?  where Id=?";
    var param = [title, author, des, keyword, focusPic, newstype, language, content, Id];
    db.query(sql, param, function (err, results) {
        if (err) {} else {
            res.json({
                msg: '操作成功',
...
router.post('/news/add', function (req, res) {
    ...
    let isShow = 0;
    let time = formatDate();
    let sql = "insert  into  news(title,author,des,keyword,newstype,focusPic,language,content,isShow,time) values(?,?,?,?,?,?,?,?,?,?)"
    var param = [title, author, des, keyword, newstype, focusPic, language, content, isShow, time];
    db.query(sql, param, function (err, results) {
        if (err) {} else {
            res.json({
                msg: '操作成功',
...
router.post('/company/add', function (req, res) {
    ...
    let content = req.body.content;
    let isShow = 0;
    let sql = "insert  into  Companyprofile(keywords,type,language,content,isShow) values(?,?,?,?,?)"
    var param = [keywords,type,language, content, isShow];
    db.query(sql, param, function (err, results) {
        if (err) {} else {
            res.json({
                msg: '操作成功',
...
router.post('/company/update', function (req, res) {
    ...
    let content = req.body.content;
    let Id = req.body.Id;
    let sql = "UPDATE Companyprofile SET keywords=?,type=?,language=?,content=?  where Id=?";
    var param = [ keywords,type, language, content, Id];
    db.query(sql, param, function (err, results) {
        if (err) {} else {
            res.json({
                msg: '操作成功',
...
router.post('/reportlist/reportlist', function (req, res) {
    ...
    let pageNo = parseInt(req.body.pageNo);
    let pageSize = parseInt(req.body.pageSize);
    let sql = "SELECT COUNT(*) FROM reportList where isShow=0 order by reportTime desc";
    let sql2 = "SELECT * FROM reportList where isShow=0 limit" + " " + (pageNo - 1) * pageSize + "," + (pageNo * pageSize);
    db.query(sql, function (err, results) {
        if (err) {
           console.log(err)
        } else {
...
router.post('/submit/messagelist', function (req, res) {
    ...
    let sql2 = "SELECT * FROM MessageBoard where isShow=0 limit" + " " + (pageNo - 1) * pageSize + "," + (pageNo * pageSize);
    db.query(sql, function (err, results) {
        if (err) {
           console.log(err)
        } else {
