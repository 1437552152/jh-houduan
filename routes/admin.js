...
router.post('/team/update', function (req, res) {
  ...
  let Id = req.body.Id;
  let sql = "UPDATE student set studentname=?,education=?,school=?,teachname=?,pic=?,introduceBriefly=?,languagelevel=?,address=?,score=?,country=?,content=? where id=?"
  var param = [studentname, education, school, teachname, pic, introduceBriefly, languagelevel, address, score, country, content, Id];
...
router.post('/team/add', function (req, res) {
  ...
  let content = req.body.content;
  let isShow = 0;
  let sql = "insert  into student(studentname,education,school,teachname,pic,introduceBriefly,languagelevel,address,score,country,content,isShow) values(?,?,?,?,?,?,?,?,?,?,?,?)"
  var param = [studentname, education, school, teachname, pic, introduceBriefly, languagelevel, address, score, country, content, isShow];
  db.query(sql, param, function (err, results) {
    if (err) {} else {
      res.json({
        msg: '操作成功',
...
router.post('/news/update', function (req, res) {
  ...
  let content = req.body.content;
  let Id = req.body.Id;
  let sql = "UPDATE news SET title=?,author=?,des=?,keyword=?,focusPic=?,newstype=?,country=?,content=?  where Id=?";
  var param = [title, author, des, keyword, focusPic, newstype, country, content, Id];
  db.query(sql, param, function (err, results) {
    if (err) {} else {
      res.json({
        msg: '操作成功',
...
router.post('/news/add', function (req, res) {
  ...
  let isShow = 0;
  let time = formatDate();
  let sql = "insert  into  news(title,author,des,keyword,newstype,focusPic,country,content,isShow,time) values(?,?,?,?,?,?,?,?,?,?)"
  var param = [title, author, des, keyword, newstype, focusPic, country, content, isShow, time];
  db.query(sql, param, function (err, results) {
    if (err) {} else {
      res.json({
        msg: '操作成功',
...
router.post('/company/add', function (req, res) {
  ...
  let content = req.body.content;
  let isShow = 0;
  let sql = "insert  into  Companyprofile(keywords,type,content,isShow) values(?,?,?,?,?)"
  var param = [keywords, type, content, isShow];
  db.query(sql, param, function (err, results) {
    if (err) {} else {
      res.json({
        msg: '操作成功',
